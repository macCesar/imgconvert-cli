/**
 * cleanup-legacy.js — context-aware removal of legacy branding artifacts.
 *
...
const { readTiapp, hasAdaptiveIcons } = require('./tiapp-reader');

/**
 * Run context-aware cleanup.
 * @param {Object} opts
 * @param {string} opts.projectRoot - Absolute project root
 * @param {'alloy'|'classic'|'unknown'} opts.projectType
 * @param {boolean} opts.aggressive - Include ldpi targets
 * @param {boolean} opts.dryRun - Print plan only
 * @returns {Promise<{removed: number, bytes: number}>}
 */
async function cleanupLegacy({ projectRoot, projectType, aggressive = false, dryRun = false }) {
  ...
  // ---- CONDITIONAL ----------------------------------------------------
  // iOS legacy launch images — storyboard enabled → not consulted
  ...
  // Android legacy default.png — adaptive icons already provide splash
