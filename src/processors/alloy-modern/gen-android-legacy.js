/**
 * gen-android-legacy.js — flat ic_launcher.png for pre-adaptive Android
 * (API 21–25, ~3% of users in 2026). Composites foreground over background,
 * scaled to legacy launcher sizes.
 *
...
/**
 * Generate legacy ic_launcher.png × 5 densities.
 * @param {string} tightMaster - Path to tight master
 * @param {string} bgColor - Hex color for background fill
 * @param {number} paddingPct - Safe-zone padding from adaptive (reduced ~40% for legacy)
 * @param {string} resRoot - Android res dir
 * @returns {Promise<string[]>} Paths to all generated files
 */
async function genAndroidLegacy(tightMaster, bgColor, paddingPct, resRoot) {
  const generated = [];
...
module.exports = { genAndroidLegacy, LEGACY_DENSITIES };
