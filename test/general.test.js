...
describe('imgconvert CLI Tests', function () {
  ...
  describe('Image resizing tests', () => {
    it('should resize image with width=300 only', () => {
      ...
      const command = `"${inputFile}" --width 300 -o "${outputDir}"`;
    ...
    it('should resize image with height=200 only', () => {
      ...
      const command = `"${inputFile}" --height 200 -o "${outputDir}"`;
    ...
    it('should resize image with width=200 and height=150', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 150 -o "${outputDir}"`;
  ...
  describe('Preset functionality tests', () => {
    ...
    it('should work with alloy preset for mobile development', () => {
      const inputFile = path.join(testDir, 'mountain-lake.png');
      const command = `alloy "${inputFile}"`;

      // Alloy outputs relative to source file directory
      const alloyOutputBase = path.join(testDir, 'app', 'assets');
      if (fs.existsSync(alloyOutputBase)) {
        fs.rmSync(alloyOutputBase, { recursive: true, force: true });
      }
      ...
      expect(checkFileExists(path.join(testDir, 'app', 'assets', 'android'), 'Android assets')).to.be.true;
      expect(checkFileExists(path.join(testDir, 'app', 'assets', 'iphone'), 'iPhone assets')).to.be.true;

      expect(checkFileExists(path.join(testDir, 'app', 'assets', 'android', 'images', 'res-mdpi'), 'Android MDPI')).to.be.true;
      expect(checkFileExists(path.join(testDir, 'app', 'assets', 'android', 'images', 'res-hdpi'), 'Android HDPI')).to.be.true;
      expect(checkFileExists(path.join(testDir, 'app', 'assets', 'iphone', 'images'), 'iPhone images')).to.be.true;
    });
  });
  ...
  describe('Error handling tests', () => {
    it('should handle invalid width parameter "invalid"', () => {
      const inputFile = path.join(testDir, 'canyon-river.jpg');
      const command = `"${inputFile}" --width invalid`;
      ...
      expect(error.stderr || error.stdout).to.match(/must be a positive integer/i);
    });

    it('should handle invalid height parameter "-5"', () => {
      const inputFile = path.join(testDir, 'canyon-river.jpg');
      const command = `"${inputFile}" --height -5`;
  ...
  describe('Advanced resizing options tests', () => {
    it('should use fit strategy "cover" to crop and fill dimensions', () => {
      ...
      const command = `"${inputFile}" --fit cover --width 200 --height 200 -o "${outputDir}"`;
    ...
    it('should use fit strategy "contain" to fit within dimensions', () => {
      ...
      const command = `"${inputFile}" --fit contain --width 200 --height 200 -o "${outputDir}"`;
    ...
    it('should use position "top" with cover fit strategy', () => {
      ...
      const command = `"${inputFile}" --fit cover --position top --width 200 --height 200 -o "${outputDir}"`;
  ...
  describe('Precedence system tests (CLI > Preset > Config > Default)', () => {
    ...
    it('should combine CLI dimensions with preset when not conflicting', () => {
      ...
      const command = `"${inputFile}" -p web --width 400 -o "${outputDir}"`;
  ...
  describe('Missing fit strategies tests', () => {
    it('should use fit strategy "fill" to stretch image to exact dimensions', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 300 --fit fill -o "${outputDir}"`;
    ...
    it('should use fit strategy "inside" to fit within dimensions preserving aspect ratio', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 300 --fit inside -o "${outputDir}"`;
    ...
    it('should use fit strategy "outside" to fill dimensions while preserving aspect ratio', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 300 --fit outside -o "${outputDir}"`;
  ...
  describe('Position values tests', () => {
    it('should use position "bottom" with cover fit strategy', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 150 --fit cover --position bottom -o "${outputDir}"`;
    ...
    it('should use position "left" with cover fit strategy', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 150 --fit cover --position left -o "${outputDir}"`;
    ...
    it('should use position "top left" with cover fit strategy', () => {
      ...
      const command = `"${inputFile}" --width 200 --height 150 --fit cover --position "top left" -o "${outputDir}"`;
  ...
  describe('Quality validation edge cases tests', () => {
    it('should handle quality value of 1 (minimum quality)', () => {
      const inputFile = path.join(testDir, 'canyon-river.jpg');
      const outputDir = path.join(testDir, 'quality-zero');
      const command = `"${inputFile}" -q 1 -o "${outputDir}"`;

      const result = execCLI(command);
      logResult('Quality one', result);
      expect(result).to.include('Processed files: 1');

      const outputFile = path.join(outputDir, 'canyon-river.jpg');
      expect(checkFileExists(outputFile, 'Min quality output')).to.be.true;
    });
  ...
  describe('Renaming integration tests', () => {
    ...
    it('should combine renaming with resizing', () => {
      ...
      const command = `"${inputDir}" --rename "prefix:thumb-" --width 150 --height 150 -o "${outputDir}"`;
  ...
  describe('Canvas resize mode tests (PNG transparency preservation)', () => {
    it('should resize PNG with transparent canvas preserving transparency', () => {
      ...
      const command = `"${inputFile}" --canvas --height 1000 -o "${outputDir}"`;
    ...
    it('should resize PNG with canvas mode and white background', () => {
      ...
      const command = `"${inputFile}" --canvas --height 1000 -b "#ffffff" -o "${outputDir}"`;
    ...
    it('should combine canvas mode with format conversion to WebP', () => {
      ...
      const command = `"${inputFile}" --canvas --height 1000 -f webp -o "${outputDir}"`;
    ...
    it('should demonstrate difference between canvas and standard resize', () => {
      ...
      const standardCommand = `"${inputFile}" --height 400 -o "${standardDir}"`;
      ...
      const canvasCommand = `"${inputFile}" --canvas --height 1000 -o "${canvasDir}"`;
    ...
    it('should work with canvas mode and custom width', () => {
      ...
      const command = `"${inputFile}" --canvas --width 1500 -o "${outputDir}"`;
    ...
    it('should work with canvas mode and both width and height', () => {
      ...
      const command = `"${inputFile}" --canvas --width 1500 --height 1000 -o "${outputDir}"`;
    ...
    it('should combine canvas mode with quality settings', () => {
      ...
      const command = `"${inputFile}" --canvas --height 1000 -f webp -q 90 -o "${outputDir}"`;
    ...
    it('should process directory with canvas mode', () => {
      ...
      const command = `"${testDir}" --canvas --height 2000 -o "${outputDir}"`;
